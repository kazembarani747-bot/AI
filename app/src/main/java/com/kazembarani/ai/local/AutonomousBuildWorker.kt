...
/** Durable on-device build loop. AI repair passes use the user's encrypted API key. */
class AutonomousBuildWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        if (request.isBlank() || planJson.isBlank()) {
            save("FAILED", "پروژه یا BuildPlan اولیه وجود ندارد.")
        ...
        setForeground(createForegroundInfo(jobId, "ساخت و تست روی دستگاه آماده می‌شود…"))
        save("RUNNING", "ساخت خودکار روی دستگاه شروع شد.")

        return try {
            val agent = LocalBuildAgent(applicationContext)
            val runtime = UnavailableBuildRuntime()
            ...
            while (System.currentTimeMillis() < deadline && attempt < 8) {
                attempt++
                setForeground(createForegroundInfo(jobId, "تلاش $attempt برای ساخت…"))
                ...
                // If the device has no local Gradle runtime yet, stop cleanly instead of pretending a build happened.
                if (!runtime.capabilities().canBuild) {
                    save("WAITING_FOR_RUNTIME", "BuildPlan آماده است، اما Runtime ساخت Android روی این دستگاه هنوز provision نشده است.\n\n${result.output}", apk?.absolutePath)
                    return Result.failure()
                }
    ...
    private fun findLatestApk(workspace: File): File? = runCatching {
        workspace.walkTopDown().filter { it.isFile && it.name.endsWith(".apk") }.maxByOrNull { it.lastModified() }?.takeIf { it.length() > 0L }
    }.getOrNull()
