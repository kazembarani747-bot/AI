...
fun LocalBuildManagerScreen(agent: LocalBuildAgent, runtime: BuildRuntime = CompanionBuildRuntime(BuildConfig.RUNTIME_URL, BuildConfig.RUNTIME_TOKEN.ifBlank { null })) {
    ...
    LazyColumn(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item {
            Text("استودیو ساخت نسخه ۱۳", style = MaterialTheme.typography.headlineSmall)
            Text("Planner → Build → Test → Diagnostics → Repair → APK")
        }
        item {
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    OutlinedTextField(
                        ...
                        minLines = 4,
                        enabled = !running,
                        placeholder = { Text("مثلاً یک اپ یادداشت با رابط فارسی بساز") }
                    )
                    Text("بودجه زمانی")
                    ...
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        Column(Modifier.weight(1f)) { Text("نصب APK پس از Build"); Text("اختیاری", style = MaterialTheme.typography.bodySmall) }
                        Switch(checked = install, onCheckedChange = { if (!running) install = it })
                    }
                    val runtimeReady = runtimeStatus?.canBuild == true
                    if (!runtimeReady) Text("🟠 Runtime ساخت در دسترس نیست. برای Build واقعی باید Runtime معتبر و متصل باشد.", color = MaterialTheme.colorScheme.error)
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Button(
                            enabled = !running && prompt.isNotBlank() && runtimeReady,
                            onClick = {
                                val id = BuildJobLauncher.enqueue(
                                    agent.context,
                                    prompt.trim(),
                                    planJson = "",
                                ...
                                running = true
                                jobState = "QUEUED"
                                jobOutput = "در صف اجرای پس‌زمینه قرار گرفت. AI در صورت نبود BuildPlan آن را می‌سازد."
                                apkPath = null
                            }
                        ) { Text(if (running) "در حال کار…" else "شروع ساخت") }
                ...
                Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    if (apk != null) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            OutlinedButton(onClick = {
                                val uri = FileProvider.getUriForFile(context, "${BuildConfig.APPLICATION_ID}.fileprovider", apk)
                                context.startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply { type = "application/vnd.android.package-archive"; putExtra(Intent.EXTRA_STREAM, uri); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION) }, "اشتراک APK"))
                            }) { Text("اشتراک") }
                        }
                    }
                }
            ...
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    when (val current = state) {
                        BuildManagerState.Idle, BuildManagerState.Inspecting -> { LinearProgressIndicator(Modifier.fillMaxWidth()); Text("در حال بررسی…") }
                        ...
                        is BuildManagerState.Success -> Text("✅ موفق")
