...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.url === "/install") { const apk = path.join(ROOT, "runtime-upload.apk"); await fs.writeFile(apk, Buffer.from(data.base64, "base64")); const r = await run(ADB, ["install", "-r", apk], ROOT, 5 * 60 * 1000); await fs.rm(apk, { force: true }); return json(res, 200, r); }
    ...
    if (req.url === "/artifact") { const artifact = path.resolve(ROOT, data.path); if (!artifact.startsWith(ROOT + path.sep)) throw new Error("path escape"); const bytes = await fs.readFile(artifact); res.writeHead(200, { "content-type": "application/vnd.android.package-archive", "content-length": bytes.length }); return res.end(bytes); }
