...
async function startSession(data) { const name = safeProjectName(data.projectName); if (!Array.isArray(data.files) || data.files.length === 0 || data.files.length > MAX_FILES) throw new Error("invalid file manifest"); let total = 0; const files = data.files.map(item => { const rel = relativeSafe(item?.path); const bytes = Number(item?.bytes); if (!Number.isSafeInteger(bytes) || bytes < 0 || bytes > MAX_FILE_BYTES) throw new Error("invalid file size: " + rel); total += bytes; return { path: rel, bytes }; }); if (total > MAX_PROJECT_BYTES) throw new Error("project exceeds maximum size"); const id = crypto.randomUUID(); const dir = sessionPath(id); await fs.mkdir(dir, { recursive: true }); await fs.writeFile(path.join(dir, "session.json"), JSON.stringify({ id, projectName: name, files, totalBytes: total, receivedBytes: 0, finalized: false }), "utf8"); return { sessionId: id, maxChunkBytes: MAX_CHUNK_BYTES, totalBytes: total }; }
async function appendChunk(data) { const { dir, meta } = await readSession(data.sessionId); if (meta.finalized) throw new Error("session already finalized"); const rel = relativeSafe(data.path); const expected = meta.files.find(f => f.path === rel); if (!expected) throw new Error("file is not in manifest"); const offset = Number(data.offset); if (!Number.isSafeInteger(offset) || offset < 0 || offset > expected.bytes) throw new Error("invalid chunk offset"); const bytes = Buffer.from(String(data.base64 || ""), "base64"); if (bytes.length === 0 && expected.bytes > 0) throw new Error("empty chunk"); if (bytes.length > MAX_CHUNK_BYTES || offset + bytes.length > expected.bytes) throw new Error("invalid chunk size"); const target = path.resolve(dir, "project", rel); if (!target.startsWith(path.resolve(dir, "project") + path.sep)) throw new Error("path escape"); await fs.mkdir(path.dirname(target), { recursive: true }); let current = 0; try { current = (await fs.stat(target)).size; } catch {} if (current !== offset) throw new Error(`offset mismatch: expected ${current}, got ${offset}`); await fs.appendFile(target, bytes); meta.receivedBytes += bytes.length; await fs.writeFile(path.join(dir, "session.json"), JSON.stringify(meta), "utf8"); return { received: bytes.length, fileReceived: offset + bytes.length, fileTotal: expected.bytes, totalReceived: meta.receivedBytes, totalBytes: meta.totalBytes }; }
async function finalizeSession(id) { const { dir, meta } = await readSession(id); for (const file of meta.files) { const target = path.join(dir, "project", file.path); let size = 0; try { size = (await fs.stat(target)).size; } catch { throw new Error("missing file: " + file.path); } if (size !== file.bytes) throw new Error(`incomplete file: ${file.path}`); } await makeGradleExecutable(path.join(dir, "project")); meta.finalized = true; await fs.writeFile(path.join(dir, "session.json"), JSON.stringify(meta), "utf8"); return { sessionId: id, projectName: meta.projectName, project: path.join(dir, "project"), totalBytes: meta.totalBytes }; }
async function gradleCommand(project, tasks, timeoutMs) { const wrapper = path.join(project, "gradlew"); try { await fs.access(wrapper); await fs.chmod(wrapper, 0o755); return await run(wrapper, tasks, project, timeoutMs); } catch { return await run(GRADLE, tasks, project, timeoutMs); } }

const server = http.createServer(async (req, res) => {
    try {
        if (req.headers.authorization !== `Bearer ${RUNTIME_TOKEN}`) return json(res, 401, { error: "unauthorized" });
        if (req.method === "GET" && req.url === "/health") {
            const adb = await adbDevices(); const gradle = await run(GRADLE, ["--version"], ROOT, 15000);
            return json(res, 200, { ok: true, runtime: "companion-runtime", build: gradle.exitCode === 0, test: gradle.exitCode === 0, install: adb.connected > 0, logcat: adb.connected > 0, screenshot: adb.connected > 0, adb: adb.result.exitCode === 0, gradle: gradle.exitCode === 0, connectedDevices: adb.connected, deviceTests: adb.connected > 0, chunkedTransfer: true, maxProjectBytes: MAX_PROJECT_BYTES });
        }
        ...
        if (req.url === "/test") { const project = projectPath(data.projectName); const tasks = safeTaskList(data.tasks); if (tasks.length === 0) return json(res, 200, { exitCode: 0, output: "No test tasks requested." }); return json(res, 200, await gradleCommand(project, tasks, 20 * 60 * 1000)); }
        if (req.url === "/test-session") { const { dir, meta } = await readSession(data.sessionId); if (!meta.finalized) throw new Error("session is not finalized"); const tasks = safeTaskList(data.tasks); if (tasks.length === 0) return json(res, 200, { exitCode: 0, output: "No test tasks requested." }); return json(res, 200, await gradleCommand(path.join(dir, "project"), tasks, 20 * 60 * 1000)); }
